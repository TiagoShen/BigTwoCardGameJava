			...
			for (int i = 0; i < maxNumOfPlayers; i++) {
				if (clientSockets[i] != null && clientOutputStreams[i] != null
					...
					try {
						...
						System.out.println("Possible connection loss: fail to broadcast message.");
//						ex.printStackTrace();
					}
				}
			}
	...
	private synchronized void addConnection(Socket clientSocket) {
		try {
			...
			if (numOfPlayers < 4) {
				...
				// only 4 players can join the game
			...
			System.out.println("Possible connection loss: fail to add connection.");
//			ex.printStackTrace();
		}
	} // addConnection
	...
	/**
	 * Creates and returns an instance of the Deck class. Overrides this method if a different type of deck is needed.
	...
	private class ClientHandler implements Runnable {
		...
		public ClientHandler(Socket clientSocket) {
			...
			try {
				...
				System.out.println("Possible connection loss: fail to get socket input stream.");
//				ex.printStackTrace();
			}
		} // constructor
